...
export const LinkPlaylistModal: React.FC<LinkPlaylistModalProps> = ({ onClose, onLink }) => {
  ...
  useEffect(() => {
    fetch('/api/playlists', {
      ...
      .catch(() => {
        toast(t('linkPlaylist.loadError'), 'error');
        setLoading(false);
      });
  }, [toast, t]);
  ...
  const handleLink = async (playlistId: number) => {
    try {
      ...
      toast(t('linkPlaylist.shareSuccess'), 'success');
      onClose();
    } catch {
      toast(t('linkPlaylist.linkError'), 'error');
    } finally {
      setLinking(null);
    }
  };
      ...
      <div className="bg-bg-card rounded-md max-w-md w-full p-6 shadow-2xl relative">
        ...
        {loading ? (
          ...
          <EmptyState 
            ...
            description={t('linkPlaylist.noPlaylistsDesc')} 
          />
        ) : (
