...
const brainDir = path.join(__dirname, 'mock_brain');
...
const watcher = chokidar.watch(path.join(brainDir, '*', '.system_generated', 'logs', 'transcript.jsonl'), {
    ignoreInitial: true,
    persistent: true
});
...
setTimeout(() => {
    ...
    setTimeout(() => {
        fs.appendFileSync(logFile, JSON.stringify({ source: 'MODEL', type: 'PLANNER_RESPONSE', content: 'Hello Slack!' }) + '\n');
    }, 1000);
}, 1000);
