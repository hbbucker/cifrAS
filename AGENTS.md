...
6. **Compilação Nativa (GraalVM & Jackson Reflection):** Para suportar a compilação nativa no Quarkus, todos os DTOs, records e models utilizados em endpoints JAX-RS ou serializados via Jackson DEVEM conter a anotação `@RegisterForReflection` ou estar registrados na classe `NativeReflectionConfig`. Sem isso, o executável nativo falha em tempo de execução (`InvalidDefinitionException: No serializer found`).
7. **Consultas a Banco & Mapeamento de Colunas:** Priorize sempre consultas orientadas a objetos com HQL/JPQL (`AdminSongEntity`, `SongEntity`) ou métodos nativos do Panache. Caso seja estritamente necessário usar SQL nativo (`em.createNativeQuery`), confirme a nomenclatura física exata das colunas no PostgreSQL (campos sem `@Column(name = "...")` são mapeados pelo Hibernate como nomes contínuos em minúsculo, como `deletedat`, `originalkey`, `userid`, e não em snake_case).
