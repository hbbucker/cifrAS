...
- **BUG-THEATER-01:** (DONE) Correção do bug de regressão do estado no Modo Teatro (E2E Test falhando no Playwright) e de warnings no ESLint. A concorrência dupla do Strict Mode do React 18 e o cache de query na preservação do tom foram resolvidos, mantendo as configurações seguras no Backend via Panache `persist()`. CI/CD 100% verde.
- **BUG-THEATER-02:** (DONE) Fix "Domino Effect" bug where transposed songs being edited (`SongViewPage.tsx` -> `SongFormPage.tsx`) were saving the transposed key as the new `originalKey` in the DB while keeping the transposition preferences, compounding the key change on next load. Fixed by sending `originalKey` instead of `transposedContent`, and added an editing warning banner to the user.
